...
async def generate_report_from_data(request: ReportRequest):
    """Generate PDF report directly from provided data without needing investigation ID."""
    ...
    meta_data = [
        [Paragraph(f"<b>TARGET ENTITY:</b> {company_name}", body_style), Paragraph(f"<b>REPORT ID:</b> DIRECT", body_style)],
        [Paragraph(f"<b>STATUS:</b> COMPLETED", body_style), Paragraph(f"<b>GENERATED:</b> {date_str}", body_style)]
    ]
    ...
    score = float(request.fraud_risk_score or 0.0)
    ...
    agent_sections = [
        ("Financial Statement Analysis", request.synthesis_data.get("financial_findings")),
        ("Flow of Funds / Graph Analysis", request.synthesis_data.get("graph_findings")),
        ("Compliance & Regulatory Checks", request.synthesis_data.get("compliance_findings")),
        ("Behavioral Audio Analysis", request.synthesis_data.get("audio_findings")),
        ("Media Sentiment & Real-time News", request.synthesis_data.get("news_findings")),
    ]
