    ...
    for query_idx, query in enumerate(queries, 1):
        ...
        with engine.connect() as conn:
            ...
            for i, row in enumerate(result, 1):
                ...
                metadata_obj = metadata if isinstance(metadata, dict) else json.loads(metadata) if isinstance(metadata, str) else {}
...
def test_rag_report_type_search():
    ...
    query_response2 = cohere_client.embed(
        texts=[query2],
        model="embed-english-v3.0",
        input_type="search_query"
    )
...
def test_cross_document_analysis():
    ...
    with engine.connect() as conn:
        ...
        if rows:
            ...
            similarity_result = conn.execute(text("""
                SELECT 
                    e1.doc_type,
                    e2.doc_type,
